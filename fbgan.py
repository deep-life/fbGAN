...
class FB_GAN():
    ...
    def __init__(self, features=DESIRED_FEATURES, generator_path=None, discriminator_path=None,
                 fbnet_path=None, multip_factor=20, score_threshold = 0.75, log_history = False, log_id=None):
        """
        Parameters
        ----------
    ...
    def get_scores(self, inputs):
        """
        Translated inputs from DNA sequence to protein sequences and outputs score for each desired feature
        defined in self.features.

        Parameters
        ----------
        inputs: list, ndarray
            list of strings with DNA sequence to be translated to a protein sequence
            shape = (N,1) where N is a number of sequences

        Returns
        -------
        scores: ndarray
            array of scores for each feature
            shape = (N, k) where k = len(self.label_order)
        """
    ...
    def get_score_per_feature(self, scores):
        ...
        scores: list,ndarray
            array of scores for each feature
            shape = (N, number_of_all_features) where number_of_all_features = len(self.label_order)

        Returns
        -------
        score_per_feature: list
            list of features and score : [['feature1', 'score'],
                                          ['feature2', 'score'],
                                                ...           ]]
            shape = (number_of_desired_features, 2) where number_of_desired_features = len(self.desired_features)
        """
    ...
    def add_samples(self, generated, scores):
        """
        Adds samples that pass the score threshold defined in self.score_threshold, adds n copies of these sequences to
        the dataset where n=self.multip_factor
        Parameters
        ----------
        generated: nadarray
            array of samples generated by a generator, one-hot encoded
            shape = (N, SEQ_LEN, N_CHAR)
        scores: ndarray
            array of scores for each feature
            shape = (N, k) where k = len(self.label_order)
    ...
    def create_dataset(self, inputs):
        ...
        -------
        tf.data.Dataset
            shuffled and batched TensorFlow dataset
        """
